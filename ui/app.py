...
class WebSocketLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        try:
            # Only emit logs from orchestrator, workflow, and adapter loggers
            ...
            print(f"[WebSocketLogHandler] Emitting: {log_entry[:100]}")
...
def execute_task():
    ...
    socketio.start_background_task(execute_task_async, actual_task, workflow, max_iterations, is_followup)
...
def execute_task_async(task: str, workflow: str, max_iterations: int, is_followup: bool = False):
    ...
    try:
        ...
        # Emit progress log
        socketio.emit('progress_log', {
            'message': f'Starting task execution with workflow: {workflow}',
            'level': 'info'
        })
        ...
        print(f"[DEBUG] WebSocketLogHandler attached. Root logger handlers: {len(root_logger.handlers)}")
